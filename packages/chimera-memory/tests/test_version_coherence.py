"""Stage 0: version coherence for the canonical v0.29.0 RC.

Guards that the CLI/dist version, the pyproject version, and the
types-dependency floor agree, and that this is the 0.29 line. Robust
...
def test_canonical_is_the_0_29_line() -> None:
    proj = _pyproject()["project"]["version"]
    assert proj.startswith("0.29"), f"expected the 0.29 RC line, got {proj!r}"
...
def test_types_dependency_floor_coheres() -> None:
    ...
    assert any("0.29" in d for d in matches), (
        f"types dependency floor should track the 0.29 line, got {matches!r}"
    )
